...
const ChatBody = (props:any) => {
    ...
    const [messages,setMessages] = useState<any[]>(props.chats)
    ...
    return (
        <div className='flex flex-col w-[calc(100%-350px)]'>
            ...
            <div className='w-full h-[calc(100%-70px-2.76rem)] overflow-y-auto p-6'>
                {messages.length>0? messages.map((x)=> <ChatMessage key = {x.id} email = {x.email} message={x.message} direction={x.email===email?"float-right":"float-left"} />):<>Nothing To see</>}
