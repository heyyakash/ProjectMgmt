    ...
    return (
        <div className=' md:overflow-auto border-r border-sec'>
            ...
            <div className='w-full p-4 md:p-6 flex flex-col gap-6'>
                {list.map(x=><Post id= {x.id} image = {x.user_image} email = {x.assigned_to} category={x.task_category} type = {name} heading={x.task_title} date = {x.deadline_date} text = {x.task_desc} key = {x.id} />)}
